...
export const signup = expressAsyncHandler(async (req, res, next) => {
  ...
  if (password.length < 5 || password.length > 20) {
    return next(
      createHttpError.BadRequest('A password must contain between 5 and 20 characters, including letters and numbers'),
    );
  }
...
export const login = expressAsyncHandler(async (req, res, next) => {
  ...
  const user = await User.findOne({ email }).populate('sandwiches').populate('parents').populate('children');
...
export const changePassword = expressAsyncHandler(async (req, res, next) => {
  ...
  user.password = await bcrypt.hash(password, parseInt(process.env.BCRYPT_SALT_ROUND, 10));
...
export const forgotPassword = expressAsyncHandler(async (req, res, next) => {
  ...
  user.resetPasswordExpire = Date.now() + parseInt(process.env.RESET_PASSWORD_EXPIRES_I, 10);
