...
const childIdValueSchema = v.pipe(
  v.string(),
  v.nonEmpty(),
  v.regex(/^[a-zA-Z0-9-_]+$/, 'Invalid child ID format')
);
const parentIdValueSchema = v.pipe(
  v.string(),
  v.nonEmpty(),
  v.regex(/^[a-zA-Z0-9-_]+$/, 'Invalid parent ID format')
);
