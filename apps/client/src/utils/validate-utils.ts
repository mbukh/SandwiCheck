import { MAX_COMMENT_LENGTH, MAX_COMMENT_LINES, MAX_NAME_LENGTH, MAX_USER_NAME_LENGTH } from '@sandwicheck/shared';
...
const validateForm = ({
  ...
  if (sandwichName != null && sandwichName.length > 0 && sandwichName.length < 3) {
    errorMessages.push('Sandwich name is too brief');
  }
  if (sandwichName != null && sandwichName.length > MAX_NAME_LENGTH) {
    errorMessages.push('Sandwich name is too long');
  }

  if (sandwichComment != null && sandwichComment.length > MAX_COMMENT_LENGTH) {
