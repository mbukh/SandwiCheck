...
type ModalCloseCallback = (event?: unknown, isProgrammatic?: boolean) => void;

interface ModalContextValue {
  registerModal: (modalId: string, closeCallback: ModalCloseCallback) => void;
...
const ModalProvider = ({ children }: { children: ReactNode }): ReactNode => {
  const activeModalReference = useRef<string | null>(null);
  const closeCallbacksReference = useRef<Map<string, ModalCloseCallback>>(new Map());

  const registerModal = useCallback((modalId: string, closeCallback: ModalCloseCallback) => {
