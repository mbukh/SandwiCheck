...
import { calculateLayerHeight } from './useStableLayerHeight.ts';
...
interface UseLayerTransitionArgs {
  isActive: boolean;
  containerRef: RefObject<HTMLElement | null>;
  skipImageEntry?: boolean;
}

interface UseLayerTransitionResult {
  ...
  measureAndPreserveHeight: () => void;
  startExitTransition: (isDeleting?: boolean, onComplete?: () => void) => void;
}
...
export const useLayerTransition = ({
  isActive,
  containerRef,
  skipImageEntry = false,
}: UseLayerTransitionArgs): UseLayerTransitionResult => {
  const [transitionState, setTransitionState] = useState<TransitionState>('idle');
  const [preservedHeight, setPreservedHeight] = useState<number | null>(null);
  const prevIsActiveRef = useRef(isActive);
  const isDeletingRef = useRef(false);
  const entryTimeoutsRef = useRef<ReturnType<typeof setTimeout>[]>([]);
  const exitTimeoutsRef = useRef<ReturnType<typeof setTimeout>[]>([]);
  ...
  const startExitTransition = useCallback(
    (isDeleting = false, onComplete?: () => void) => {
  ...
  /*
   * Timer-driven enter/exit animation state machine reacting to `isActive` changes. These
   * setState calls drive the animation sequence (synchronising with timers) — a legitimate
   * effect, not a "derived state in effect" smell, so the rule is disabled for this block.
   */
  ...
  const showControls = isActive && transitionState === 'controls-in' && !isExiting;
