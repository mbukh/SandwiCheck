  ...
  const confirmHandler = (e) => {
    ...
    if (isCurrentlyBread) {
      sandwichDispatch({
        ...
        payload: [{ ...currentIngredient, portion: DEFAULT_PORTION }, ...sandwich.ingredients.slice(1)],
      });
    } else {
      sandwichDispatch({
        type: 'ADD_INGREDIENT',
              ...
              ingredientPlace.isPresent
                ? {
                    opacity:
                      0.4 +
                      0.2 *
                        Object.values(PORTION).indexOf(
                          sandwich.ingredients.find((ingredient) => ingredient.id === currentIngredient.id).portion,
                        ),
                  }
                : {}
            }
