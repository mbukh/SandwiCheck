...
import SandwichIngredientsList from '../Card/SandwichIngredientsList';

const SandwichSaveModal = ({ isOpen, onClose }) => {
  ...
  const sandwichNameInputRef = useRef(null);
  ...
  // Update modal key when opening to force remount
  useEffect(() => {
    if (isOpen) {
  ...
  // Keep expanded if there's content when modal opens
  useEffect(() => {
    if (isOpen) {
      if (sandwich.comment && sandwich.comment.trim().length > 0) {
        setIsCommentExpanded(true);
      ...
      // Reset warning state and sync previous value when modal opens
      setPreviousCommentValue(sandwich.comment || '');
      setShowNewlineWarning(false);
    }
  }, [isOpen, sandwich.comment]);
