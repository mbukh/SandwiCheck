...
import SwiperNavigationButton from '@/components/Sandwich/SwiperNavigationButton';
import SwiperSlideElementNone from '@/components/Sandwich/SwiperSlideElementNone';
...
interface LayerSwiperProps {
  isAddingLayer?: boolean;
  editingLayerIndex?: number;
  onSwiperReady?: (moveToFirstSlide: () => void) => void;
  className?: string;
}
...
const LayerSwiper = ({
  ...
  onSwiperReady,
  className,
}: LayerSwiperProps): React.JSX.Element | null => {
  const [navigation, setNavigation] = useState<NavigationState>({ prev: false, next: true });
  ...
  const currentType = isAddingNewLayer
    ? selectedType || (editingLayerIndex === null ? undefined : sandwich.ingredients[editingLayerIndex]?.type)
    : editingLayerIndex === null
      ? ''
      : sandwich.ingredients[editingLayerIndex]?.type;
  const ingredientsOfType = useMemo<SandwichLayer[]>(
    () => (currentType ? allIngredients[currentType] || [] : []),
  ...
  useEffect(() => {
    ...
    if (isVisible && !isAddingLayer && !isEditingAddedLayer && editingLayerIndex !== null && initialSlide >= 0) {
      ...
      if (initialSlide === 0 && hasNoneSlide) {
        ...
        const ingredientAtIndex = ingredientsOfType[ingredientIndex];
        if (ingredientAtIndex) {
          setCurrentIngredient(ingredientAtIndex);
        }
      }
    }
    ...
    if (
      ...
      const firstIngredient = ingredientsOfType[0];
      if (firstIngredient) {
        setCurrentIngredient(firstIngredient);
      }
    }
  }, [
    isVisible,
  ...
  const updateNavigationButtons = (activeIndex: number): void => {
    const hasNoneSlide = !isAddingNewLayer;
    const totalSlides = ingredientsOfType.length + (hasNoneSlide ? 1 : 0);

    const navUpdate: Record<number, NavigationState> = {
    ...
    const nextNavigation = navUpdate[start + end];
    if (nextNavigation) {
      setNavigation(nextNavigation);
    }
  };

  const initSwiperHandler = (swiper: SwiperClass): void => {
  ...
  const slideChangeHandler = (swiper: SwiperClass): void => {
    const hasNoneSlide = !isAddingNewLayer;
    const noneSlideOffset = hasNoneSlide ? 1 : 0;
              ...
              const portion =
                isAddingNewLayer && contextCurrentIngredient?.portion
                  ...
                  : editingLayerPortion && !isAddingNewLayer
                    ? editingLayerPortion
                    : ingredient.portion;
              const ingredientWithPortion: SandwichLayer = portion ? { ...ingredient, portion } : ingredient;
              return (
